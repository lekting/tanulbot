/**
 * Message handler for processing user text messages
 */
import { Context, InputFile } from 'grammy';
import { createMainMenu, createDifficultyMenu, createDiaryMenu, getKeyboardActions, KEYBOARD_IDS, createLanguageMenu } from '../bot/keyboards';
import { correctAndReply, correctAndReplyWithWords } from '../services/openai/index';
import { generateDictationPhrases, generateDictationPhrasesByDifficulty } from '../services/dictation';
import { getUserLevel, compareHungarianTexts } from '../utils/text';
import { store } from '../store';
import { DictationDifficulty } from '../types';
import { 
  processDiaryEntry, 
  createAnkiDeck, 
...
export async function handleTextMessage(ctx: Context): Promise<void> {
  ...
  if (messageText === actions.STOP_DIARY) {
    ...
    await ctx.reply(t('diary.saved', userLang), { reply_markup: createMainMenu(userLang) });
    
    const entries = store.getUserDiaryEntries(userId);
    if (entries.length > 0) {
      // Process the last entry
      const lastEntry = entries[entries.length - 1];
      const processed = await processDiaryEntry(lastEntry);
      ...
      if (mnemonicMessages.length > 0) {
        await ctx.reply(
          t('diary.mnemonics', userLang, { mnemonics: mnemonicMessages.join('\n\n') }),
    ...
    await ctx.reply(t('general.choose_action', userLang), { reply_markup: createMainMenu(userLang) });
    return;
  }
  ...
  if (messageText === actions.GENERATE_ANKI) {
    ...
    if (entries.length === 0) {
      await ctx.reply(t('anki.need_diary', userLang), { reply_markup: createMainMenu(userLang) });
      return;
    }

    // Show processing message
    await ctx.reply(t('anki.creating', userLang), { reply_markup: createMainMenu(userLang) });
    
    try {
      console.log(`Starting Anki deck creation for user ${userId} with ${entries.length} entries`);
      const ankiDeck = await createAnkiDeck(entries, userId);
      console.log(`Anki deck created successfully: ${ankiDeck.filePath}`);
      
      const suggestions = await generateLearningSuggestions(entries);
      ...
      if (ankiDeck.filePath) {
        ...
        try {
          ...
          await ctx.replyWithDocument(
            ...
            { caption: t('anki.file_caption', userLang, { count: ankiDeck.words.length }) }
          );
          ...
          const errorDetails = fileError instanceof Error ? fileError.message : 'Unknown error';
          await ctx.reply(t('anki.error', userLang, { 
            message: `Error sending the Anki deck file: ${errorDetails}` 
          }), { reply_markup: createMainMenu(userLang) });
        }
      } else {
        console.warn('No file path in the Anki deck object');
        await ctx.reply(t('anki.error', userLang, { message: 'Anki deck was created but the file is not available for download.' }), 
          { reply_markup: createMainMenu(userLang) }
        );
      }
  ...
  if (messageText === actions.CLEAR_CHAT) {
    ...
    await ctx.reply(t('chat.cleared', userLang), { reply_markup: createMainMenu(userLang) });
    return;
  }
  ...
  if (messageText === actions.VIEW_CHAT) {
    ...
    if (chatHistory.length === 0) {
      await ctx.reply(t('chat.empty', userLang), { reply_markup: createMainMenu(userLang) });
      return;
    }
    ...
    await ctx.reply(`${t('chat.history', userLang)}\n\n${formattedHistory}`, { reply_markup: createMainMenu(userLang) });
    return;
  }

  if (messageText === actions.START_DICTATION) {
    await ctx.reply(t('general.choose_difficulty', userLang), { reply_markup: createDifficultyMenu(userLang) });
    return;
  }
  ...
  if (Object.keys(difficultyMap).includes(messageText)) {
    ...
    // Generate phrases based on difficulty
    const phrases = await generateDictationPhrasesByDifficulty(difficulty);
    store.startDictation(userId, { currentIndex: 0, phrases, points: 0, difficulty });

    await ctx.reply(t('dictation.start', userLang), { reply_markup: createMainMenu(userLang) });
  ...
  if (messageText === actions.STOP_DICTATION) {
    ...
    await ctx.reply(t('dictation.stopped', userLang), { reply_markup: createMainMenu(userLang) });
    return;
  }
  ...
  if (messageText === actions.VIEW_VOCABULARY) {
    ...
    if (vocabulary.length === 0) {
      await ctx.reply(t('vocabulary.empty', userLang), { reply_markup: createMainMenu(userLang) });
      return;
    }
    ...
    const message = `${t('vocabulary.title', userLang, { count: totalWords })}\n\n${formattedWords}` + 
      (totalWords > displayedWords ? `\n\n(${t('general.showing_limited', userLang, { shown: displayedWords, total: totalWords })})` : '');
  ...
  if (store.hasDictation(userId)) {
    ...
    if (compareHungarianTexts(messageText, current.text)) {
      ...
      const difficultyMultiplier = state.difficulty === 'hard' ? 3 : state.difficulty === 'medium' ? 2 : 1;
      ...
      await ctx.reply(t('dictation.incorrect', userLang, { text: current.text }));
    }
  ...
  const response = await correctAndReplyWithWords(messageText, userLang, formattedHistory, CHAT_HISTORY_TOKENS);
  ...
  if (response.words && response.words.length > 0) {
    ...
    if (wordsCount === 1) {
      ...
      wordsAddedMsg = t('vocabulary.words_added', userLang, { count: wordsCount });
    }
    
    await ctx.reply(response.text + (wordsAddedMsg ? '\n\n' + wordsAddedMsg : ''), 
      { reply_markup: createMainMenu(userLang) });
  } else {
